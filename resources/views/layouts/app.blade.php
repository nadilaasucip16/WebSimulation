...
<nav class="main-nav">
  ...
  @if(request()->routeIs('fase4'))
  {{-- Accessed from Activity Fase 4: show Needham context badge --}}
  <span style="font-family:var(--mono);font-size:11px;color:var(--amber);
               border:1px solid rgba(255,180,84,.35);background:rgba(255,180,84,.07);
               padding:4px 12px;border-radius:20px;margin-left:8px;white-space:nowrap;">
    Needham · Fase 4 — Aplikasi
  </span>
  @else
  {{-- Direct/standalone access: show nav links --}}
  <div class="nav-links">
    <a href="{{ route('blueprint.builder') }}"
       class="nav-link {{ request()->routeIs('blueprint.*') ? 'active' : '' }}">Blueprint Builder</a>
    <a href="{{ route('simulasi.index') }}"
       class="nav-link {{ request()->routeIs('simulasi.*') ? 'active' : '' }}">Skenario</a>
  </div>
  @endif
  ...
  <div class="nav-right">
    ...
    <a href="{{ route('dashboard.siswa') }}"
       style="font-size:12px;color:var(--dim);padding:5px 12px;border-radius:6px;
              border:1px solid var(--border);text-decoration:none;transition:.15s;white-space:nowrap;"
       onmouseover="this.style.color='var(--text)';this.style.borderColor='#3a4254';"
